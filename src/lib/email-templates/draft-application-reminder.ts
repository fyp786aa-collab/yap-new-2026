import { emailBaseLayout } from "./base-layout";

export function draftApplicationReminderTemplate(name = "Candidate"): string {
  const content = `
    <h2 style="color: #152232; font-size: 22px; font-weight: 600; margin: 0 0 16px;">
      Reminder: Complete and Submit Your YAP 2026 Application
    </h2>
    <p style="color: #334155; font-size: 15px; line-height: 1.7; margin: 0 0 16px;">
      Dear ${name},
    </p>
    <p style="color: #334155; font-size: 15px; line-height: 1.7; margin: 0 0 16px;">
      This is a reminder that your Young Ambassador Programme (YAP) 2026 application is still in <strong>Draft</strong> status.
    </p>
    <p style="color: #334155; font-size: 15px; line-height: 1.7; margin: 0 0 16px;">
      Please complete all pending sections and submit your application before the deadline:
      <strong>Last Date to Apply: 11 April 2026</strong>.
    </p>
    <div style="background-color: #f8fafc; border-left: 4px solid #82a845; border-radius: 0 8px 8px 0; padding: 20px 24px; margin: 24px 0;">
      <h3 style="color: #152232; font-size: 16px; font-weight: 600; margin: 0 0 12px;">
        Before submitting, make sure you:
      </h3>
      <ul style="color: #334155; font-size: 14px; line-height: 1.8; margin: 0; padding-left: 20px;">
        <li>Upload all required documents (including CV and transcript).</li>
        <li>Upload your introductory video.</li>
        <li>Review the detailed guidelines available on the application portal.</li>
      </ul>
    </div>
    <p style="color: #334155; font-size: 15px; line-height: 1.7; margin: 0 0 24px;">
      Portal link:
      <a href="https://yap-new-2026.vercel.app" style="color: #82a845; text-decoration: none; font-weight: 600;">
        https://yap-new-2026.vercel.app
      </a>
    </p>
    <div style="margin-top: 32px; padding-top: 24px; border-top: 1px solid #e2e8f0;">
      <p style="color: #334155; font-size: 15px; line-height: 1.7; margin: 0;">
        Warm regards,
      </p>
      <p style="color: #152232; font-size: 15px; font-weight: 600; line-height: 1.7; margin: 4px 0 0;">
        AKYSB Pakistan
      </p>
      <p style="color: #82a845; font-size: 14px; margin: 2px 0 0;">
        Young Ambassador Programme Team
      </p>
    </div>
  `;

  return emailBaseLayout(content);
}
